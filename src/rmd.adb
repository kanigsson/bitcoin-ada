...
package body RMD is
   ...
   is
      Result: Word;
   begin
      if    J <= 15 then Result := X xor Y xor Z;
      elsif J <= 31 then Result := (X and Y) or ((not X) and Z);
      elsif J <= 47 then Result := (X or not Y) xor Z;
      elsif J <= 63 then Result := (X and Z) or (Y and not Z);
   ...
   is
      K: Word;
   begin
      if    J <= 15 then K := 16#0000_0000#;
      elsif J <= 31 then K := 16#5A82_7999#;
      elsif J <= 47 then K := 16#6ED9_EBA1#;
      elsif J <= 63 then K := 16#8F1B_BCDC#;
      else                           K := 16#A953_FD4E#;
      end if;
      return K;
   end K_L_Spark;
   ...
   is
      K: Word;
   begin
      if    J <= 15 then K := 16#50A2_8BE6#;
      elsif J <= 31 then K := 16#5C4D_D124#;
      elsif J <= 47 then K := 16#6D70_3EF3#;
      elsif J <= 63 then K := 16#7A6D_76E9#;
      else                           K := 16#0000_0000#;
      end if;
      return K;
   end K_R_Spark;
   ...
   begin
      ...
      loop
         -- left
         T := Wordops.Rotate(S_L_Spark(J),
                             CLA +
                               F_Spark(J, CLB, CLC, CLD) +
                               X(R_L_Spark(J)) +
                               K_L_Spark(J)) +
           CLE;
         CLA := CLE;
         ...
         CLB := T;
         -- right
         T := Wordops.Rotate(S_R_Spark(J),
                             CRA +
                               F_Spark(79 - J, CRB, CRC, CRD) +
                               X(R_R_Spark(J)) +
                               K_R_Spark(J)) +
           CRE;
         CRA := CRE;
   ...
   begin
      ...
      loop
         Round_Spark(CA, CB, CC, CD, CE, X(I));
